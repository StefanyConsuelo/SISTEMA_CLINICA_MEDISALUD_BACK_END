...
public interface ExamenRespository {
	...
	@Select("select te.examen_id, ts.nombre servicio, te.resultado, te.historia_clinica_id,"
			...
			+ "	thc.paciente_id, DATE_FORMAT(te.fecha_creacion,'%d-%m-%Y') as fecha_creacion from tb_examen te"
			...
			+ "	where te.historia_clinica_id  = #{historiaClinicaId}"
			+ "	order by te.examen_id desc limit #{inicio},#{fin};" )
	List<Examen> listarPaginado( Integer inicio, Integer fin, Integer historiaClinicaId );
