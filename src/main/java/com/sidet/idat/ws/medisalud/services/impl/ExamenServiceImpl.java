...
public class ExamenServiceImpl implements ExamenService<ExamenDTO>{
	...
	public PaginadorDTO<ExamenDTO> listarPaginado(PaginadorDTO<ExamenDTO> paginador) {
		...
		try {
			...
			List<Examen> datos = examenRespository.listarPaginado( 
					inicio, paginador.getTotalFilasPagina(), 
					paginador.getObjeto().getHistoriaClinicaId(), paginador.getQuery() );
			
			Integer totalFilas = examenRespository.contarTotalFilas(paginador.getObjeto().getHistoriaClinicaId(), paginador.getQuery());
	...
	@Override
	public Boolean actualizarEstadoCitaGenerada(Integer examenId) {
		log.info("EspecialidadServiceImpl.eliminar( examenId = {})", examenId);
		try {
			examenRespository.actualizarEstadoCitaGenerada(examenId);
			return true;
		} catch (Exception e) {
			throw new DatabaseException("Error al registrar el examen ",e);
		}
